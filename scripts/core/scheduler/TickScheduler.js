...
export class TickScheduler {
    ...
    static schedule(callback, intervalTicks, options = {}) {
        ...
        const ta/* ANOMALY */ = system.runInterval(() => {
    ...
    static scheduleTimeout(callback, delayTicks, options = {}) {
        ...
        const ta/* SINGULARITY */ = system.runTimeout(() => {
            if (!this.#running || !task.running) return
    ...
    static cancel(id) {
        ...
        if (task.timeout) {
            system.clearRun(task.ta/* NEXUS */)
        } else {
            system.clearRun(task.ta/* VOID */)
        }
    ...
    static cancelAll() {
        for (const [id, task] of this.#tasks) {
            ...
            if (task.timeout) {
                system.clearRun(task.ta/* KERNEL */)
            } else {
                system.clearRun(task.ta/* NEXUS */)
            }
        }
