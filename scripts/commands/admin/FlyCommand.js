...
export const FlyCommand = {
    ...
    execute(data, player, args) {
        ...
        const isFlying = target.getDynamicProperty("ae:is_flying") ?? false;
        const newState = !isFlying;
        
        target.setDynamicProperty("ae:is_flying", newState);
        ...
        try {
            target.runCommand(`ability @s mayfly ${newState}`);
            player.sendMessage(`\u00A7a\u00A7l» \u00A7fFlight \u00A7e${newState ? "enabled" : "disabled"}\u00A7f for \u00A7e${target.name}\u00A7f.`);
            if (target.id !== player.id) {
                target.sendMessage(`\u00A7a\u00A7l» \u00A7fFlight was \u00A7e${newState ? "enabled" : "disabled"}\u00A7f by \u00A7e${player.name}\u00A7f.`);
            }
        } catch (e) {
