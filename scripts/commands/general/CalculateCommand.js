...
export const CalculateCommand = {
    ...
    /**
     * Executes the mathematical calculation command.
     * 
     * EXPECTS:
     * - player: The Player object executing this command.
     * - args: Array containing the raw math expression at args[0].
     * 
     * GUARANTEES:
     * - Parses and evaluates the math expression securely.
     * - Sends result directly to player's chat.
     * - Safely intercepts division by zero or malformed syntax.
     * 
     * @param {any} _data - Unused command registry context.
     * @param {import("@minecraft/server").Player} player - Executing player.
     * @param {string[]} args - Expression parameters.
     */
...
/**
 * Checks if operator is unary based on position context.
 * 
 * EXPECTS:
 * - index: Current token index position.
 * - tokens: Token list array.
 * 
 * GUARANTEES:
 * - Returns true if operator behaves as unary (e.g., negative prefix).
 * 
 * @param {number} index - Current token index.
 * @param {string[]} tokens - Expression tokens list.
 * @returns {boolean} Whether operator is unary.
 */
 ...
 * 
 * EXPECTS:
 * - expression: Mathematical string formula.
 * 
 * GUARANTEES:
 * - Checks parentheses balance.
 * - Resolves expression to a single floating point number.
 * - Throws Error on unbalanced parentheses, unknown operators, or invalid math values.
 * 
 * @param {string} expression - Math formula.
 ...
 * 
 * EXPECTS:
 * - expr: Sanitized math formula string.
 * 
 * GUARANTEES:
 * - Splits string into separate tokens representing numbers, constants, functions, and symbols.
 * 
 * @param {string} expr - Formula string.
 * @returns {string[]} Tokenized formula segments.
 */
...
/**
 * Converts infix token list to Reverse Polish Notation (Postfix).
 ...
 * 
 * GUARANTEES:
 * - Returns array of tokens ordered in postfix notation.
 * - Handles mathematical operator precedence rules.
 * 
 * @param {string[]} tokens - Infix token segments.
 * @returns {any[]} Postfix tokens array.
 */
...
/**
 * Resolves a postfix RPN expression using a value stack.
 * 
 * EXPECTS:
 * - rpn: Postfix token list.
 * 
 * GUARANTEES:
 * - Resolves the postfix stack to a single final number.
 * - Catches overflow values and illegal functions.
 * 
 * @param {any[]} rpn - Postfix tokens array.
 * @returns {number} The evaluated final result.
 */
